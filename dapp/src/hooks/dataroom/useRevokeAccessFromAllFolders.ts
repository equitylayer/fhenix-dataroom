...
export function useRevokeAccessFromAllFolders(dataRoomAddress: HexAddress | undefined) {
	...
	const revokeAccessFromAllFolders = useCallback(
		async (parentId: bigint, user: string) => {
			...
			try {
				...
				console.error("revokeAccessFromAllFolders failed:", e);
				setIsPending(false);
				setIsConfirming(false);
